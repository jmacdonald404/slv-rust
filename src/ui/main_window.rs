...
pub fn show_main_window(ctx: &egui::Context, ui_state: &mut UiState) {
    ...
    if let Ok(login_result) = ui_state.login_result_rx.try_recv() {
        match login_result.result {
            Ok(_) => {
                ...
                start_world_connection(ui_state);
            }
            Err(error) => {
    ...
    // Check for UDP connection results
...
fn start_login(ui_state: &mut UiState) {
    ...
    ui_state.login_task = Some(ui_state.runtime_handle.spawn(async move {
        match crate::ui::login::logic::perform_login(&username, &password, selected_grid, &proxy_settings).await {
            Ok(_) => {
...
fn start_world_connection(ui_state: &mut UiState) {
