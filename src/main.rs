...
use slv_rust::networking::protocol::messages::{PacketHeader, Message};
...
async fn main() {
    ...
    // Server side
    let server_circuit = Circuit::new(&server_listen_addr.to_string(), "0.0.0.0:0".parse().unwrap()).await.unwrap();
    let server_handle = tokio::spawn(async move {
        ...
        server_circuit.send_message(&received_header, &received_message, &sender_addr).await.unwrap();
    ...
    // Client side
    let client_circuit = Circuit::new(client_listen_addr, server_listen_addr).await.unwrap();

    let header = PacketHeader { sequence_id: 1, flags: 0 };
    let message = Message::KeepAlive;

    client_circuit.send_message(&header, &message, &server_listen_addr).await.unwrap();
    ...
    println!("Client received echo: {:?}, {:?}", received_header, received_message);
